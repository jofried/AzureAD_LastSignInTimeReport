...
try 
{
    $tokenRequest = Invoke-WebRequest -Method Post -Uri "https://login.microsoftonline.us/$tenantId/oauth2/v2.0/token" -ContentType "application/x-www-form-urlencoded" -Body $body -UseBasicParsing -ErrorAction Stop
}
catch 
{ 
...
#######################################################################
$SignInActivity = Invoke-WebRequest -Headers $AuthHeader -Uri "https://graph.microsoft.us/beta/users?`$select=displayName,userPrincipalName,signInActivity"
